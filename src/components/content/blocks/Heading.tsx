...
export function Heading({ level, children, align = 'left' }: HeadingProps) {
  ...
  return (
    <Tag className={`font-bold mt-6 mb-2 ${sizeClasses[level as keyof typeof sizeClasses]} ${alignClass}`}>
      {children}
    </Tag>
  );
}
...
export function MainTitle({ children, align = 'left' }: { children: React.ReactNode, align?: 'left' | 'center' | 'right' }) {
  const alignClass = align === 'center' ? 'text-center' : align === 'right' ? 'text-right' : '';
  return (
    <h1 className={`text-4xl font-bold text-gray-800 mb-1 ${alignClass}`}>
      {children}
    </h1>
  );
}
