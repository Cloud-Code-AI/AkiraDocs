...
import { Tabs, TabsContent, TabsList, TabsTrigger } from 'akiradocs-ui';
import { ClipboardIcon, CheckIcon, PlusIcon, MinusIcon } from '@heroicons/react/24/outline';
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter';
import { oneLight } from 'react-syntax-highlighter/dist/esm/styles/prism';
import { oneDark } from 'react-syntax-highlighter/dist/esm/styles/prism';
import { Card, CardHeader, CardTitle, CardContent } from 'akiradocs-ui';
...
export function ApiUsage({ apiSpec }: { apiSpec: any }) {
  ...
  const [includedParams, setIncludedParams] = useState<Record<string, string[]>>({});
                      ...
                      {optionalParameters.map((param: any) => (
                        <div key={param.name} className="flex items-center space-x-2">
                          <ParameterToggle
                            ...
                            onToggle={(paramName: string) => toggleParam(endpointId, paramName)}
                          />
